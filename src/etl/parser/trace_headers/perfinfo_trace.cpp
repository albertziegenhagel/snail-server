...
std::size_t perfreader::etl::parser::parse(binary_stream_parser& stream, perfinfo_trace_header& data)
{
    auto read_bytes = parse(stream, data.version);
    read_bytes += parse(stream, data.header_type);
    assert(data.header_type == trace_header_type::perfinfo32 ||
           data.header_type == trace_header_type::perfinfo64);

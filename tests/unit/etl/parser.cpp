...
TEST(EtlParser, Kernel_ProcessV2TypeGroup2EventView)
{
    const std::array<std::uint8_t, 104> buffer = {
        0x0c, 0x04, 0x00, 0x00, 0xb0, 0x07, 0x00, 0x00, 0x46, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0xf0, 0x2e, 0x03, 0x01, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x30, 0x19, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0xda, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0xc8, 0x1e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x16, 0x03, 0x01, 0x00, 0x00, 0x00,
        0x00, 0xd0, 0x13, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x50, 0x16, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x38, 0x71, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x1d, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x50, 0x16, 0x00, 0x00, 0x00, 0x00, 0x00};

    const auto event = etl::parser::process_v2_type_group2_event_view(std::as_bytes(std::span(buffer)), 8);

    EXPECT_EQ(event.dynamic_size(), event.buffer().size());

    EXPECT_EQ(event.process_id(), 1036);

    EXPECT_EQ(event.page_fault_count(), 1968);
    EXPECT_EQ(event.handle_count(), 70);

    EXPECT_EQ(event.reserved(), 0);

    EXPECT_EQ(event.peak_virtual_size(), 4348375040);
    EXPECT_EQ(event.peak_working_set_size(), 4255744);
    EXPECT_EQ(event.peak_pagefile_usage(), 1650688);
    EXPECT_EQ(event.quota_peak_paged_pool_usage(), 55816);
    EXPECT_EQ(event.quota_peak_non_paged_pool_usage(), 7880);

    EXPECT_EQ(event.virtual_size(), 4346802176);
    EXPECT_EQ(event.working_set_size(), 1298432);
    EXPECT_EQ(event.pagefile_usage(), 1462272);
    EXPECT_EQ(event.quota_paged_pool_usage(), 28984);
    EXPECT_EQ(event.quota_non_paged_pool_usage(), 7472);

    EXPECT_EQ(event.private_page_count(), 1462272);
}
